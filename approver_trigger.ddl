...
begin
  ...
  from F15C7EMP where F15C7LABS_LAB_ID = :new.F15C7LABS_LAB_ID and LAB_DIRECTOR_FLAG = 1 and employee_id != current_seq and employee_status = 'A';
  select count(*) into count_sys_admins
  from F15C7EMP where F15C7LABS_LAB_ID = :new.F15C7LABS_LAB_ID and SYSTEM_ADMIN_FLAG = 1 and employee_id != current_seq and employee_status = 'A';
  select count(*) into count_exec_directors
  from F15C7EMP where EXEC_DIRECTOR_FLAG = 1 and employee_id != current_seq and employee_status = 'A';
  select count(*) into count_chair_persons
  from F15C7EMP where CHAIR_PERSON_FLAG = 1 and employee_id != current_seq and employee_status = 'A';
  ...
  if count_sys_admins > 0 and :new.system_admin_flag = 1 then
    ...
    raise_application_error(-20001, 'There is already a lab director for that lab.');
  elsif count_exec_directors > 0 and :new.exec_director_flag = 1 then
    raise_application_error(-20001, 'There is already an executive director.');
  elsif count_chair_persons > 0 and :new.chair_person_flag = 1 then
    raise_application_error(-20001, 'There is already a chairperson.');
  end if;
end;
/
...
COMPOUND TRIGGER
  ...
  before each row is
  begin
    if :new.employee_id != active_exec_director AND :new.exec_director_flag = 1 then
...
COMPOUND TRIGGER
  ...
  before each row is
  begin
    if :new.employee_id != active_chair_person AND :new.chair_person_flag = 1 then
